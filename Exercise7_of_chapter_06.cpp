...
constexpr char constant='C';    //added
const string constkey="const";  //added

constexpr char help='H';    //added
...
public:
    string name;
    double value;
    bool isconst;   //changing the data structure

    //Constructor
    Variable(string n,double v,bool ic=false): name{n},value{v},isconst{ic} { }
};
...
{
private:
    vector<Variable> var_table; // The locked vault of variables
public:
    double get(string s)
    {
    ...
    void setv(string s,double d)
    {
        for(Variable& v: var_table)
        {
            if(v.name==s)
            {
                if(v.isconst)
                {
                    error(s, " is a constant and cannot be changed");
                }
                v.value=d;
                return;
            }
    ...
    double declare(string var,double val,bool isconst=false)
    {
...
try
{
    cout << "Welcome to our simple calculator!" << '\n'
        ...
        << "To exit, type exit" << '\n'
        << "Type 'H' or'h' or 'help' to see the help menu" << '\n';
